...
#magicscroll_images tr td input[type="text"] {
    width: 133px;
    margin: 5px 0;
}
#magicscroll_images tr td textarea {
    width: 133px;
    min-width: 133px;
    max-width: 133px;
    margin: 5px 0;
}
#magicscroll_images tr td select {
    width: 38px;
}
#magicscroll_upload_list {
    border-bottom: none;
}
#magicscroll_upload_list tr td:first-child {
   width: 180px;
}
#magicscroll_upload_list tr td input[type="text"], #magicscroll_upload_list tr td textarea {
    margin: 5px 0;
}
...
<form id="magicscroll_settings" action="<?php echo htmlentities($_SERVER['REQUEST_URI']); ?>" method="post" enctype="multipart/form-data">
...
<input class="button" type="button" value="<?php echo $this->l('Save settings'); ?>" onclick="document.getElementById('magic_submit').click();" />
    ...
    <div id="<?php echo $blockId; ?>_content" class="magic_tabCont<?php if($blockId == 'default') echo ' magic_active'; ?>">
        ...
        <legend><img alt="" src="../img/admin/contact.gif" /><?php echo $groupTitle; ?></legend>
            ...
            <?php
            if($id == 'magicscroll') {
                echo "<img id=\"magicscroll_icon\" src=\""._MODULE_DIR_."magicscroll/admin_graphics/magicscroll.png\" />";
            }
                ...
                case "array":
                    if($tool->params->getSubType($id, $tool->params->generalProfile) == 'radio') {
                        foreach($tool->params->getValues($id) as $p) {
                            ...
                            $pLower = strtolower($p);
                            if($pLower == "yes")
                                echo '<img src="../modules/magicscroll/admin_graphics/yes.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" />';
                            elseif($pLower == "no")
                                echo '<img src="../modules/magicscroll/admin_graphics/no.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" />';
                            elseif($pLower == "left")
                                echo '<img src="../modules/magicscroll/admin_graphics/left.gif" alt="'.$this->l('Left').'" title="'.$this->l('Left').'" />';
                            elseif($pLower == "right")
                                echo '<img src="../modules/magicscroll/admin_graphics/right.gif" alt="'.$this->l('Right').'" title="'.$this->l('Right').'" />';
                            elseif($pLower == "top")
                                echo '<img src="../modules/magicscroll/admin_graphics/top.gif" alt="'.$this->l('Top').'" title="'.$this->l('Top').'" />';
                            elseif($pLower == "bottom")
                                echo '<img src="../modules/magicscroll/admin_graphics/bottom.gif" alt="'.$this->l('Bottom').'" title="'.$this->l('Bottom').'" />';
                            else echo $p;
                            ?></label><?php
                        }
...
<script src="<?php echo _MODULE_DIR_ ?>magicscroll/options.js" type="text/javascript"></script>
<script type="text/javascript">
    //<![CDATA[
