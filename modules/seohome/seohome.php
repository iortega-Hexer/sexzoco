...
class SEOHOME extends Module
{
	public function __construct()
	{
		$this->name = 'seohome';
		$this->description = 'SEO TOP HOME WITH H1';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Seohome_MONOLANGUAGE';
		$this->text_editor_content = 'Seohome_TEXTEDITOR';
		$this->content_wrapper = 'Seohome_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Seohome_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Seohome_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('seo_home');
		$this->description = $this->l('SEO TOP HOME WITH H1');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !SEOHOMEModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = SEOHOMEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = SEOHOMEModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					SEOHOMEModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					SEOHOMEModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																SEOHOMEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class SEOHOMEModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( SEOHOMEModel::createFilesTable()
				&& SEOHOMEModel::createContentTable());
	}
