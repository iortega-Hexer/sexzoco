...
class fmeblockmanufacturer extends Module
{
    public function __construct()
    {
        $this->name = 'fmeblockmanufacturer';
		...
		$this->displayName = $this->l('FME Manufacturers block');
        $this->description = $this->l('Displays a scroller of manufacturers/brands');
    }
	...
	public function hookDisplayHome($params)
	{
		$this->smarty->assign(array(
			...
			'brand_image' => Configuration::get('MANUFACTURER_IMAGE'),
		));
		return $this->display(__FILE__, 'fmeblockmanufacturer.tpl');
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitFmeBlockManufacturers'))
		{
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset><legend><img src="'.$this->_path.'logo.gif" alt="" title="" />'.$this->l('Settings').'</legend>
				<div class="margin-form">
					...
					<input type="radio" style="display:none" name="text_list" id="text_list_off" value="0" '.(!Tools::getValue('text_list', Configuration::get('MANUFACTURER_DISPLAY_TEXT')) ? 'checked="checked" ' : '').'/>
					<label class="t" for="text_list_off" style="display:none"> <img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" /></label>
					&nbsp;&nbsp;&nbsp;'.$this->l('Scroll').' <input type="text" size="2" name="text_nb" value="'.(int)(Tools::getValue('text_nb', Configuration::get('MANUFACTURER_DISPLAY_TEXT_NB'))).'" /> '.$this->l('elements').'
				...
				<center><input type="submit" name="submitfmeblockmanufacturers" value="'.$this->l('Save').'" class="button" /></center>
			</fieldset>
		</form>';
		return $output;
	}
	...
	{
		$this->context->controller->addCSS($this->_path.'fmeblockmanufacturer.css');
